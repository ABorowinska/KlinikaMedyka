...
const config = {
    dbfilename: './data/przychodnia.sqlite3',
    LICZBA_PACJENTOW: 50,
    LICZBA_LEKARZY: 10,
    LICZBA_GABINETOW: 15,
    LICZBA_WIZYT: 150
...
const insertPacjent = connection.prepare('INSERT OR IGNORE INTO pacjenci (imie, nazwisko, pesel, data_urodzenia, email) VALUES (?, ?, ?, ?, ?)');
connection.transaction(() => {
    for (let i = 0; i < config.LICZBA_PACJENTOW; i++) {
        ...
        const email = faker.internet.email({ firstName: imie, lastName: nazwisko }).toLowerCase();

        insertPacjent.run(imie, nazwisko, pesel, data_urodzenia, email);
    }
})();
...
const specjalizacje = ['Kardiolog', 'Pediatra', 'Chirurg', 'Okulista', 'Dermatolog', 'Neurolog', 'Stomatolog', 'Ortopeda', 'Urolog'];
const insertLekarz = connection.prepare('INSERT OR IGNORE INTO lekarze (imie, nazwisko, specjalizacja, email, gabinet_id) VALUES (?, ?, ?, ?, ?)');
connection.transaction(() => {
...
const insertWizyta = connection.prepare('INSERT INTO wizyty (pacjent_id, lekarz_id, gabinet_id, data, status, uwagi_do_statusu) VALUES (?, ?, ?, ?, ?, ?)');
connection.transaction(() => {
    for (let i = 0; i < config.LICZBA_WIZYT; i++) {
        const pacjent_id = faker.number.int({ min: 1, max: config.LICZBA_PACJENTOW });
        const lekarz_id = faker.number.int({ min: 1, max: config.LICZBA_LEKARZY });
        const gabinet_id = faker.number.int({ min: 1, max: config.LICZBA_GABINETOW });
        ...
        const godzina = String(faker.number.int({ min: 8, max: 16 })).padStart(2, '0');
        const minuty = String(faker.helpers.arrayElement([0, 15, 30, 45])).padStart(2, '0');
        ...
        insertWizyta.run(pacjent_id, lekarz_id, gabinet_id, sformatowanaData, status, uwagi);
    }
})();
...
const insertAudyt = connection.prepare('INSERT INTO audyt (uzytkownik, operacja, szczegoly) VALUES (?, ?, ?)');
insertAudyt.run('System', 'INICJALIZACJA', 'Wygenerowano nową bazę danych przychodni.');
