    ...
    (req: Request, res: Response) => {
        ...
        try {
            if (!imie || !nazwisko || !pesel) {
                return res.status(400).json({
                    sukces: false,
                    wiadomosc: 'Brakuje wymaganych pól (imię, nazwisko, PESEL)'
                });
            }
            ...
            const dodajPacjenta = db.transaction(() => {
                ...
                const osobaId = Number(osobaInfo.lastInsertRowid);
                ...
                return {
                    osobaId,
                    pacjentId: Number(pacjentInfo.lastInsertRowid)
                };
            });
...
router.patch(
    ...
    (req: Request, res: Response) => {
        ...
        try {
            if (czy_aktywny !== 0 && czy_aktywny !== 1) {
                return res.status(400).json({
                    sukces: false,
                    wiadomosc: 'Nieprawidłowa wartość czy_aktywny'
                });
            }

            const zmienStatusPacjenta = db.transaction(() => {
                const pacjent = db.prepare(`
                    SELECT id, osoba_id
                    FROM pacjenci
                    WHERE id = ?
                `).get(id) as
                    | { id: number; osoba_id: number }
                    | undefined;
            ...
            const pacjent = zmienStatusPacjenta();

            if (!pacjent) {
                return res.status(404).json({
                    sukces: false,
                    wiadomosc: 'Nie znaleziono pacjenta'
                });
            }
            ...
            return res.json({
                sukces: true,
                wiadomosc: 'Status pacjenta został zaktualizowany'
            });
        } catch (error: any) {
...
router.put(
    ...
    (req: Request, res: Response) => {
        ...
        try {
            ...
            const aktualizujPacjenta = db.transaction(() => {
                const pacjent = db.prepare(`
                    SELECT id, osoba_id
                    FROM pacjenci
                    WHERE id = ?
            ...
            const zaktualizowano = aktualizujPacjenta();

            if (!zaktualizowano) {
                return res.status(404).json({
                    sukces: false,
                    wiadomosc: 'Nie znaleziono takiego pacjenta w bazie'
                });
            }
            ...
            return res.json({
                sukces: true,
                wiadomosc: 'Dane pacjenta zaktualizowane'
            });
        } catch (error: any) {
            console.error('Błąd aktualizacji pacjenta:', error);
